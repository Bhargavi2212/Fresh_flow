...
@tool
def check_order_inventory(order_items_json: str, customer_id: str) -> str:
    """
    ...
    Returns:
        JSON string with checked_items (each with availability_status, available_quantity,
        shortfall_quantity, suggested_substitutions, triggers_reorder), procurement_signals,
        and summary (available/partial/out_of_stock counts).
    """
...
Use check_stock for each item. Use find_substitutions for any partial or out_of_stock. Use get_expiring_items for items expiring within 2 days. Return a single JSON object only (no markdown)."""
    result = inventory_agent.invoke(prompt)
